...
- 后端 `leads_query_users` 支持 page / page_size / lead_type / keyword 参数。
- 后端 `final_leads_query_users` 支持 page / page_size / final_action / keyword 参数。
- 后端 `export_final_leads_csv` 支持 `final_actions` 过滤，可按 action 组合导出 Final Lead。
- 前端 Lead Center 增加后端查询控件：keyword、lead_type、final_action、page、page size。
- 导出预设从“只改文件名”升级为“文件名 + Final Action 过滤范围”联动。
- Market Upsell 导出限定 `MARKET_FIBER_UPSELL` / `FTTH_SPEED_UPSELL`。
- Reachability 导出限定 `REACHABILITY_FIX_FIRST` / `BUILD_OR_COVERAGE_CHECK`。
...
1.0.10 继续补齐 Lead 查询与导出能力：SA Lead / Final Lead 查询不再固定前端一次取 100 条，后端支持分页和过滤；Final Lead 导出支持按 action 组合过滤。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。
