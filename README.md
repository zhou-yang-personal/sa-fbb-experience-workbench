...
- 新增 Quality Center 独立面板，集中展示质量门禁动作与检查范围。
- 新增 ETL Job Center 独立面板，集中管理 Jobs、Steps、失败步骤和 ETL 执行链。
- 新增 Dashboard Center 独立面板，集中进入 Overview、App Category、Experience Quality、Cable/FTTH 和 Final Summary。
- 新增 Final Lead Center 独立面板，集中处理 SA Lead、Final Lead、Final Action Mix 和导出。
- WorkbenchAppV2 从单一 Operations 区演进为 Import / Quality / ETL / Dashboard / Final Lead 分区工作台。
...
1.0.8 继续补齐前端工作台分区：Quality Center、ETL Job Center、Dashboard Center 和 Final Lead Center 已落地。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。`src-tauri/tauri.conf.json` 仍因 connector safety block 未同步到 1.0.8。
