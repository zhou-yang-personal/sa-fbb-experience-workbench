...
- 数据集画像从单一 row_count 扩展为 row_count、source_line_range、distinct_accounts、distinct_macs 和数据类型特定维度。
- 新增 mapping validation 结果查询与汇总 command。
- Import Center 增加映射汇总、映射结果、刷新画像、查看画像入口。
- Workbench header、package、Cargo、README、handoff、changelog 与项目规则同步到 1.0.7。
...
1.0.7 继续补齐 Import Center 的“导入前校验 / 导入后画像 / 映射结果复核”闭环。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。`src-tauri/tauri.conf.json` 仍因 connector safety block 未同步到 1.0.7。
