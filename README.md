...
- 主看板的接入类型、关键词和最小用户数筛选改为后端执行，并在 `LIMIT` 前生效；搜索结果不再受固定 Top N 截断。
- App、小时、网络、用户和机会证据查询返回 `returned_rows`、`has_more`、`effective_limit`、来源表和筛选摘要。
- “问题 App”改为统计样本充分且持续差体验用户数大于 0 的 V2 App；没有 V2 聚合时明确显示不可用，不再回退到“差体验占比大于 0”。
- KPI 使用筛选范围内的后端全量聚合；屏幕与 PDF 共用同一查询计划和图表定义，不再各自进行前端二次筛选。
- 不修改数据库结构、依赖和 lock 文件；保留显式加载和零数据库访问启动边界。
...
1.0.55 已把主看板、结构化分页面板和 PDF 收敛到同一后端筛选语义，并用 V2 持续差体验口径替代旧问题 App 判定。真实 Windows/MySQL 1320 万行下的筛选性能、PDF 一致性和第 201 位之后关键词回归仍需在本地数据环境验收。
