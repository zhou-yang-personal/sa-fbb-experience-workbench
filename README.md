...
- 质量报告新增 `CLEAN conversion` 卡片，展示 `raw_rows`、`clean_rows` 和 `clean_to_raw_ratio`。
- 当 CLEAN 行数为 0、明显低于 RAW、或超过 RAW 时，质量报告会给出 `not_cleaned` / `low_yield` / `over_cleaned` 状态，帮助定位 RAW → CLEAN 清洗丢数或重复问题。
- 保留 1.0.2 的 CSV vs RAW 行数一致性检查、RAW 类型分布检查和导入/ETL 状态查询能力。
...
1.0.3 继续修复前一轮 review 中的任务可观测性问题：导入进度、行数一致性、ETL 状态查询、RAW 类型分布和 RAW → CLEAN 转换率。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。当前分支仍应作为完整应用代码基线，后续必须以本地编译和真实数据修复为准。
