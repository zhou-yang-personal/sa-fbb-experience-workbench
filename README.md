...
Backend: Rust
Database: MySQL 8.0
CSV Import: LOAD DATA LOCAL INFILE + fallback design
Package manager: npm
```
...
1.0.0 已按 Phase 1-7 把完整应用设计主路径落地到 `dev`，但没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。当前分支应作为完整应用代码基线，后续必须以本地编译和真实数据修复为准。
