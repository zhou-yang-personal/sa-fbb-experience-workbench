...
1.0.59 将总览与应用体验重构为“全量洞察 + 问题高亮”，并为结构化看板查询增加 NULL 安全解码、命令级崩溃隔离和只读加载边界；1.0.56 的 hourly V2 分片、checkpoint、互斥锁和恢复日志能力保持不变。真实 Windows/MySQL 1320 万行的看板查询与完整恢复仍需在目标机器验收。
