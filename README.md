...
- Final Lead 结果新增查询 command 和前端列表入口。
- Final Lead 支持全量分页 CSV 导出，不再只依赖 SA Lead 导出。
- CRM / Coverage / Reachability 融合 SQL 增加商业资格动作：黑名单剔除、欠费检查、合约检查、可触达修复、覆盖建设检查。
- Final Lead UI 同时展示 final action、CRM ID、FTTH 可用状态和可触达状态。
...
1.0.4 继续修复 Phase 6/7 闭环：最终营销名单查询、最终名单导出和商业资格动作已经落地。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。当前分支仍应作为完整应用代码基线，后续必须以本地编译和真实数据修复为准。
