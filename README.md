...
1.0.55 已把数据操作收敛为新建导入、批次库和运行任务三个显式工作区；批次选择、任务加载和日志刷新不再混成隐式操作。1.0.54 的空值安全解码与 1.0.52 的 RAW 重建、SQL 逐语句日志继续保留。真实 Windows/MySQL 1320 万行的交互、刷新稳定性和重建耗时仍需在本地数据环境验收。
