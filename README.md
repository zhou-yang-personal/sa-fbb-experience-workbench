...
- Import Center 数据类型从 TCP / Game 扩展到 TCP / Game / CRM / FTTH Coverage / Reachability。
- RAW 导入支持 CRM、覆盖和可触达三类商业辅助数据表。
- 商业辅助 RAW 表补充 `source_file_name` 和 `source_line_no`，保持可追溯性。
- Lead CSV 导出从固定前 1000 条改为分页全量导出。
...
1.0.3 继续修复 Phase 6 和导出闭环缺口：商业辅助 CSV 的 RAW 入库入口和 Lead 全量分页导出已落地。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。当前分支仍应作为完整应用代码基线，后续必须以本地编译和真实数据修复为准。
