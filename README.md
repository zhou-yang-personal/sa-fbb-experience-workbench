...
- App 详情新增“判定依据 → 规则门槛 → 分子/分母”证据链，并明确 App 用户质差不等于 App 服务端根因。
- Cable / FTTH 新增五维体验雷达摘要；固定业务锚点、保留原始值，任一维度缺失时不按 0 分闭合图形。
- 所有用户分档与 Cable / FTTH 分档改为人数 + 占比柱图；制式对比使用各自群体内占比，避免总体规模差异造成视觉误导。
- 潜客列表和详情新增逐用户入选原因、逐项门槛与优先级解释；当前规则只有 High / Standard，没有虚构综合分数。
- 小时曲线使用分析运行绑定的高峰区间，并在缺失小时断线，避免跨缺口连接出不存在的趋势。

## 4.1 1.0.72 聚合恢复与发布完整性修复

- 旧 checkpoint 的 `source_version=NULL` 现在会安全判定为不可复用，不再触发 Rust 后台线程 panic。
- 聚合子任务与小时分片同时校验实现版本和来源版本；公共小时/周期核心升级到 V4，首次正式恢复会重新计算旧 V3 小时分片，补齐新增速率、RTT 和丢包累加字段。
- 任一上游子任务重算时，所有下游 checkpoint 自动失效；后台未捕获异常会把 pipeline、step、analysis run 和活动 checkpoint 收口为失败或中断。
- 全景、Cable/FTTH、潜客和导出增加精确发布门禁：运行中、旧版本、缺失 checkpoint 或混合发布结果不再被读取。
...
- CSV 包含用户 IP、机会类型、优先级、接入制式、流量、时长、速率、RTT、丢包、Wi-Fi、主 App、证据说明、数据限制和规则版本。
- 后端逐行读取已发布潜客 ADS 并直接写入目标文件，不将全部名单加载到前端内存；文件使用 UTF-8 BOM，便于 Windows Excel 直接打开。

## 4.1 1.0.70 全景、制式专项与潜客证据

- 全景指标视角新增流量、时长、上下行速率可用性、视频有效速率、时延和丢包的连续分析区：整体口径、用户分档、小时变化、App 排序和问题高亮。
- Cable / FTTH 专项新增整体绝对/相对差异、24 小时双曲线、用户分布和同 App 双侧样本对比；页面只读取预聚合 ADS。
- 潜客页新增按 IP 分页的四类候选列表、类型筛选、IP/App 搜索和证据详情弹框；普通页面移除长事务“生成潜客”按钮。
- 潜客生成拆为用户特征、四类候选和原子发布六个可恢复子任务；小时大表不再被潜客任务重复扫描。
- 流量、时长、观测数、速率、RTT 和丢包分档边界进入版本化规则高级配置；流量换算统一为 `1 TB = 1000 GB`。
- 进入业务分析页后自动读取有界 DWS/ADS，不再反复要求点击“加载”；自动读取不会导入 CSV、扫描 RAW/DWD 或启动聚合任务。

## 4.1 1.0.69 App 弹框与视角去重
