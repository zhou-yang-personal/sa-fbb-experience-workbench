...
1.0.13 继续收口旧入口残留：当前主入口仍是 `WorkbenchAppV2`，旧 `App.tsx` / `WorkbenchApp.tsx` 仅保留兼容转发，`OpsPanel` / `TaskCenter2` 仅保留 legacy placeholder。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。
