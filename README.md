...
1.0.56 已把 hourly V2 改为带索引、checkpoint 和互斥锁的日期/小时分片任务，并统一恢复路径的逐 SQL 日志。用户可复用已有 RAW/CLEAN/Period V2 结果，从未完成小时分片继续；真实 Windows/MySQL 1320 万行的分片耗时、索引构建耗时和完整恢复仍需在目标机器验收。
