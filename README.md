...
- 版本标记已同步到 `package.json`、`src-tauri/tauri.conf.json`、`WorkbenchHeader.tsx`、`mapping_catalog.rs`、README、handoff、changelog；`src-tauri/Cargo.toml` 在 ChatGPT GitHub connector 中被 safety block 拦截，需由本地或 Codex 后续补齐到 `1.0.29`。
...
- 本轮继续复用既有 DWS / ADS 后端 commands，不直接扫 RAW，不新增依赖，不修改数据库结构。
...
1.0.29 在 1.0.28 结构化证据表基础上，把主驾驶舱改为 structured-first chart/data source：优先消费结构化 Analytics ADS API，并保留旧 dashboard commands fallback。编译验证和 smoke 结果以最终统一验证为准；真实 MySQL / customer CSV smoke 如未执行，不得视为通过。
