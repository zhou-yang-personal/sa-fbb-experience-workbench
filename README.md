...
- 新增结构化 ETL Step 明细模型 `EtlJobStepRow`。
- 新增后端 command：`etl_get_job_steps`。
- ETL Step 明细支持按 import_batch_id 查询，并可选按 job_id、status、limit 过滤。
- 前端 `jobApi` 新增 `jobSteps` 调用封装。
- `JobStepActions` 增加 Step Detail / Failed Detail 查询入口。
- `EtlJobCenter` 增加结构化 Step 详情表，展示 job_type、job_id、step_name、status、target_table、affected_rows、started_at、finished_at、message。
...
1.0.11 继续补齐 ETL 可观测性：ETL Step 不再只能作为 MetricCard 查看，已支持结构化详情表和失败步骤明细入口。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。
