...
- Streaming INSERT fallback 会在导入过程中持续更新 `meta_import_batch.imported_rows`。
- RAW 导入完成时会同步 `total_rows` 和 `imported_rows`，用于行数一致性检查。
- 质量门禁新增 `csv_vs_raw_rows` 检查，输出 `total_rows` 与 `imported_rows` 差异。
- 新增导入状态查询命令和前端“刷新导入状态”。
- 新增 ETL 最近任务查询命令和前端“刷新ETL状态”。
...
1.0.2 继续修复前一轮 review 中的任务可观测性问题：导入进度、行数一致性、ETL 状态查询。没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。当前分支仍应作为完整应用代码基线，后续必须以本地编译和真实数据修复为准。
