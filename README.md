...
- RAW 导入改为 TCP / Game 显式字段列表，降低 CSV 列错位风险。
- RAW 导入支持 `LOAD DATA LOCAL INFILE` 和 streaming INSERT fallback 两条路径。
- `mode=streaming_insert` 或 `local_infile=false` 时走流式分块 INSERT。
- ETL 命令开始写入 `meta_etl_job` 和 `meta_etl_job_step`，支持步骤状态、失败信息和后续重跑排查。
- RAW 质量门禁扩展到时间范围、小时分布、应用数量和拓扑 UNKNOWN 检查。
...
1.0.1 已继续修复前一轮 review 中最关键的偏差：RAW 导入字段映射、fallback 入口、ETL 作业日志和质量门禁覆盖。但没有在本地执行依赖安装、前端构建、Rust 检查、Tauri 打包和真实 MySQL / CSV 端到端验证。当前分支仍应作为完整应用代码基线，后续必须以本地编译和真实数据修复为准。
