...
1.0.20 已把 Import / Mapping / RAW Load 主链路改为后端契约收口。旧库 catalog 会自动刷新，mapping fail 不再留下 pending batch。编译验证和 smoke 结果以本轮交付报告与 changelog 的实际执行记录为准；真实 MySQL / customer CSV smoke 如未执行，不得视为通过。
