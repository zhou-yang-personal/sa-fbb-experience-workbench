...
UNION ALL
SELECT analysis_run_id, 'video_gb', 'Video GB', CAST(COALESCE(ROUND(SUM(total_download_gb),2),0) AS CHAR), 'total_download_gb' FROM p, params
UNION ALL
SELECT analysis_run_id, 'game_hours', 'Game Hours', CAST(COALESCE(ROUND(SUM(total_game_hours),2),0) AS CHAR), 'total_game_hours' FROM p, params
UNION ALL
