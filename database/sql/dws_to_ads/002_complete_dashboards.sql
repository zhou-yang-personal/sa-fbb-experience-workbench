...
SELECT params.analysis_run_id, COALESCE(a.app_category, 'UNKNOWN') AS app_category, COALESCE(a.user_type, 'UNKNOWN') AS user_type,
       SUM(a.active_users), SUM(a.total_download_gb), SUM(a.total_game_hours), AVG(COALESCE(a.avg_vmos, a.avg_mos))
FROM :dws_app_category_daily a, params
WHERE a.import_batch_id = params.import_batch_id
GROUP BY params.analysis_run_id, COALESCE(a.app_category, 'UNKNOWN'), COALESCE(a.user_type, 'UNKNOWN');
