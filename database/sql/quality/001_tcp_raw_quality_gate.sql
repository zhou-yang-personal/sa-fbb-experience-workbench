...
WITH params AS (SELECT :import_batch_id AS import_batch_id), batch AS (
  ...
  SELECT 'tcp' AS data_type,
         ...
         COUNT(DISTINCT NULLIF(NULLIF(TRIM(universal_video_applications), ''), '--')) AS app_cnt,
...
SELECT import_batch_id, 'raw_quality', 'tcp_row_count', 'row_cnt', row_cnt, NULL,
       CASE WHEN row_cnt = 0 THEN 'error' ELSE 'info' END,
       CASE WHEN row_cnt = 0 THEN 0 ELSE 1 END
FROM raw_counts, params
UNION ALL
SELECT p.import_batch_id, 'raw_quality', 'tcp_csv_vs_raw_rows', 'row_diff', COALESCE(b.total_rows, 0) - COALESCE(b.imported_rows, 0),
...
UNION ALL
SELECT import_batch_id, 'raw_quality', 'tcp_identity', 'user_account_cnt', user_account_cnt, CONCAT('user_mac_cnt=', user_mac_cnt),
...
FROM raw_counts, params
UNION ALL
SELECT import_batch_id, 'raw_quality', 'tcp_empty_account_pct', 'empty_account_pct',
...
FROM raw_counts, params
UNION ALL
SELECT import_batch_id, 'raw_quality', 'tcp_access_type_mix', 'cable_rows', cable_rows, CONCAT('ftth_rows=', ftth_rows),
       CASE WHEN cable_rows = 0 OR ftth_rows = 0 THEN 'warning' ELSE 'info' END,
       CASE WHEN cable_rows = 0 OR ftth_rows = 0 THEN 0 ELSE 1 END
FROM raw_counts, params
UNION ALL
SELECT import_batch_id, 'raw_quality', 'tcp_time_range', 'active_hours', active_hours, CONCAT('min_time=', COALESCE(CAST(min_time AS CHAR), 'NULL'), ', max_time=', COALESCE(CAST(max_time AS CHAR), 'NULL'), ', invalid_time_rows=', invalid_time_rows),
...
FROM raw_counts, params
UNION ALL
SELECT import_batch_id, 'raw_quality', 'tcp_app_count', 'app_cnt', app_cnt, NULL,
       CASE WHEN app_cnt = 0 THEN 'error' WHEN app_cnt < 3 THEN 'warning' ELSE 'info' END,
       CASE WHEN app_cnt = 0 THEN 0 ELSE 1 END
FROM raw_counts, params
UNION ALL
SELECT import_batch_id, 'raw_quality', 'tcp_topology_unknown_pct', 'unknown_bras_pct',
...
FROM raw_counts, params
UNION ALL
SELECT import_batch_id, 'clean_quality', 'tcp_clean_row_count', 'clean_rows', clean_rows,
       CONCAT('ok_rows=', ok_rows),
       CASE WHEN clean_rows = 0 THEN 'warning' ELSE 'info' END,
       1
FROM clean_counts, params
UNION ALL
SELECT import_batch_id, 'clean_quality', 'tcp_unknown_user_key_pct', 'warn_unknown_user_key_pct',
...
FROM clean_counts, params
UNION ALL
SELECT import_batch_id, 'clean_quality', 'tcp_invalid_stat_time_pct', 'warn_invalid_stat_time_pct',
...
FROM clean_counts, params
UNION ALL
SELECT import_batch_id, 'clean_quality', 'tcp_unknown_access_type_pct', 'warn_unknown_access_type_pct',
