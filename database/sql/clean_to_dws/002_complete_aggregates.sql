...
WITH params AS (SELECT :import_batch_id AS import_batch_id), video AS (
  SELECT import_batch_id, stat_date, user_type, app_category,
  ...
  GROUP BY import_batch_id, stat_date, user_type, app_category
), game AS (
  SELECT import_batch_id, stat_date, user_type, app_category,
         COUNT(DISTINCT user_key) AS active_users,
...
WITH params AS (SELECT :import_batch_id AS import_batch_id), video AS (
  SELECT import_batch_id, stat_date, hour_of_day, user_type,
         COUNT(DISTINCT user_key) AS active_users,
         AVG(vmos) AS avg_vmos,
         NULL AS avg_mos,
  ...
  FROM dwd_tcp_detail_clean WHERE import_batch_id = (SELECT import_batch_id FROM params)
  GROUP BY import_batch_id, stat_date, hour_of_day, user_type
), game AS (
  SELECT import_batch_id, stat_date, hour_of_day, user_type,
         COUNT(DISTINCT user_key) AS active_users,
         NULL AS avg_vmos,
