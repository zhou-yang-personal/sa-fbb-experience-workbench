...
INSERT INTO :dws_user_app_period_experience_v2 (
  ...
  total_effective_duration_hours, total_video_duration_hours, active_days,
  avg_effective_download_mbps, avg_vmos, avg_mos,
  avg_download_mbps, avg_throughput_mbps, avg_max_single_flow_mbps,
  avg_connection_success_pct, avg_connection_delay_ms, avg_download_fluency,
  avg_upstream_rtt_ms, avg_downstream_rtt_ms,
  avg_user_up_loss_pct, avg_network_up_loss_pct,
  avg_wifi_delay_ms,
...
tcp_observations AS (
  SELECT
    ...
    COALESCE(d.effective_duration_hours, 0) AS effective_duration_hours,
    COALESCE(d.video_duration_hours, 0) AS video_duration_hours,
    d.effective_download_mbps,
    d.avg_download_mbps,
    d.throughput_mbps,
    d.max_single_flow_mbps,
    d.connection_success_pct,
    d.connection_delay_ms,
    d.download_fluency,
    d.upstream_rtt_ms,
    d.downstream_rtt_ms,
    d.user_up_loss,
...
game_observations AS (
  SELECT
    ...
    CAST(0 AS DECIMAL(24,6)) AS effective_duration_hours,
    CAST(0 AS DECIMAL(24,6)) AS video_duration_hours,
    CAST(NULL AS DECIMAL(18,6)) AS effective_download_mbps,
    CAST(NULL AS DECIMAL(18,6)) AS avg_download_mbps,
    CAST(NULL AS DECIMAL(18,6)) AS throughput_mbps,
    CAST(NULL AS DECIMAL(18,6)) AS max_single_flow_mbps,
    CAST(NULL AS DECIMAL(18,6)) AS connection_success_pct,
    CAST(NULL AS DECIMAL(18,6)) AS connection_delay_ms,
    CAST(NULL AS DECIMAL(18,6)) AS download_fluency,
    CAST(NULL AS DECIMAL(18,6)) AS upstream_rtt_ms,
    CAST(NULL AS DECIMAL(18,6)) AS downstream_rtt_ms,
    CAST(NULL AS DECIMAL(18,6)) AS user_up_loss,
    CAST(NULL AS DECIMAL(18,6)) AS network_up_loss,
    d.wifi_delay_ms,
...
user_app AS (
  SELECT
    ...
    SUM(f.effective_duration_hours) AS total_effective_duration_hours,
    SUM(f.video_duration_hours) AS total_video_duration_hours,
    COUNT(DISTINCT f.stat_date) AS active_days,
    AVG(f.effective_download_mbps) AS avg_effective_download_mbps,
    AVG(f.avg_download_mbps) AS avg_download_mbps,
    AVG(f.throughput_mbps) AS avg_throughput_mbps,
    AVG(f.max_single_flow_mbps) AS avg_max_single_flow_mbps,
    AVG(f.connection_success_pct) AS avg_connection_success_pct,
    AVG(f.connection_delay_ms) AS avg_connection_delay_ms,
    AVG(f.download_fluency) AS avg_download_fluency,
    AVG(f.upstream_rtt_ms) AS avg_upstream_rtt_ms,
    AVG(f.downstream_rtt_ms) AS avg_downstream_rtt_ms,
    AVG(f.user_up_loss) AS avg_user_up_loss_pct,
    AVG(f.network_up_loss) AS avg_network_up_loss_pct,
    AVG(f.wifi_delay_ms) AS avg_wifi_delay_ms,
...
SELECT
  ...
  u.total_download_gb,
  u.total_game_hours,
  u.total_effective_duration_hours,
  u.total_video_duration_hours,
  u.active_days,
  u.avg_effective_download_mbps,
  u.avg_vmos,
  u.avg_mos,
  u.avg_download_mbps,
  u.avg_throughput_mbps,
  u.avg_max_single_flow_mbps,
  u.avg_connection_success_pct,
  u.avg_connection_delay_ms,
  u.avg_download_fluency,
  u.avg_upstream_rtt_ms,
  u.avg_downstream_rtt_ms,
  u.avg_user_up_loss_pct,
  u.avg_network_up_loss_pct,
  u.avg_wifi_delay_ms,
