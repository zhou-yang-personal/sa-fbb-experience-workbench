...
-- SA remains the opportunity source; CRM / coverage / reachability only decide final action.
...
SELECT
  ...
  CASE
    ...
    WHEN b.lead_type LIKE 'A1_%' AND COALESCE(MAX(cv.ftth_available_flag), 'UNKNOWN') = 'Y'
      AND (MAX(r.phone_available_flag) = 'Y' OR MAX(r.sms_available_flag) = 'Y' OR MAX(r.app_push_available_flag) = 'Y') THEN 'MARKET_FIBER_UPSELL'
