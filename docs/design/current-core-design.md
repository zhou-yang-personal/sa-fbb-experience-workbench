...
Cable/FTTH 五维雷达只做执行摘要，采用固定业务锚点并同时显示原始值；缺失维度不按 0 计分，也不闭合该侧多边形。精确结论继续以总体差异、同小时双曲线、群体内分档占比和同 App 双侧充分样本为证据。分档柱长使用各自分母下的占比，人数作为伴随值，避免群体总体规模不同造成视觉误导。

打印报告必须从布局树中折叠交互页面，只保留报告 DOM；禁止仅用 `visibility:hidden` 隐藏大段页面。小时趋势从分析运行绑定规则读取高峰区间，缺失小时断线，不得跨缺口制造连续趋势。

## 1.0.70 制式与潜客公共底座

Cable/FTTH 专项先从周期核心生成 `dws_user_access_period_v2`，整体和用户分档再从该小表派生；小时专项只扫描共享小时核心一次，并生成 Cable、FTTH 与 ALL 小时 ADS。普通页面只读这些有界结果。

潜客生成先把周期核心和用户日表收敛为每个分析 IP 一行的 `dws_user_opportunity_feature_v2`。迁转、升套、Mesh 和 App Bundle 分别写入 staging 并记录独立 checkpoint；全部成功后在一个短事务内替换正式用户表和汇总表。旧成功结果在发布前持续可读，页面不得直接触发重计算。

全景和制式用户分档边界随 `meta_decision_rule_profile` 版本保存。页面显示值、分子、分母、单位和不可用状态；可复用平均指标必须使用公共核心的 SUM/COUNT，而不是对下一级平均值再次简单 AVG。

## 1.0.69 全景视角与 App 下钻

全景交互收敛为指标与 App 两个互补视角。用户分档继续由指标视角中的有界 DWS/ADS 查询提供，不再通过独立页签重复加载相同结果。唯一 App 清单的行点击在当前视口打开模态详情，异步读取该 App 的用户分布，并隔离并发请求；旧请求完成时不得覆盖操作者后来选择的 App。详情弹框必须呈现加载、失败和空结果状态，支持 Esc、遮罩和显式按钮关闭。
