...
pub fn analytics_get_hourly_trend(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    ...
    if let Ok(ads_table) = batch_tables::resolve_table(&req.settings, &req.import_batch_id, "ads_hourly_experience_trend") {
        ...
        if ads_count.unwrap_or(0) > 0 {
            ...
            return conn.exec_map(sql, (&run_id, keyword.clone(), keyword.clone(), keyword, req.min_value(), page_size, offset), |(hour, user_type, app_category, users, mbps, rtt, loss): (i64, String, String, i64, f64, f64, f64)| MetricCard {
                label: format!("h{hour:02} {user_type} {app_category}"),
                value: users.to_string(),
                hint: format!("source=ads_hourly_experience_trend, hour={hour}, user_type={user_type}, app_category={app_category}, users={users}, effective_mbps={mbps:.2}, subscriber_rtt_ms={rtt:.2}, user_loss_pct={loss:.2}, page_size={page_size}, offset={offset}"),
    ...
    conn.exec_map(sql, (&req.import_batch_id, keyword.clone(), keyword, req.min_value(), page_size, offset), |(hour, user_type, users, mbps, rtt, loss): (i64, String, i64, f64, f64, f64)| MetricCard {
