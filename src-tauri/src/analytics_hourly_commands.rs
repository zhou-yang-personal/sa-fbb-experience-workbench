...
fn analytics_get_hourly_trend_inner(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    ...
    if let Ok(ads_table) = batch_tables::resolve_table(
        ...
        if ads_count.unwrap_or(0) > 0 {
            ...
            return conn.exec_map(sql, (&run_id, keyword.clone(), keyword.clone(), keyword, req.min_value(), page_size, offset), |(stat_date, hour, user_type, app_category, users, mbps, rtt, loss): (String, i64, String, String, i64, f64, f64, f64)| MetricCard {
