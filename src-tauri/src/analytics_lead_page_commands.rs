...
pub fn analytics_get_lead_evidence_page(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    ...
    let ads_detail = batch_tables::resolve_table(&req.settings, &req.import_batch_id, "ads_lead_evidence_detail")?;
    let ads_count: Option<i64> = conn.exec_first(format!("SELECT CAST(COUNT(*) AS SIGNED) FROM `{ads_detail}` WHERE analysis_run_id=?"), (&run_id,)).map_err(|err| format!("failed to count analytics lead ADS: {err}"))?;
