...
fn analytics_get_lead_evidence_page_inner(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    ...
    if let Ok(ads_detail) = batch_tables::resolve_table(
        ...
        if ads_count.unwrap_or(0) > 0 {
            ...
            return conn.exec_map(sql, (&run_id, keyword.clone(), keyword.clone(), keyword.clone(), keyword, req.min_value(), page_size, offset), |(user_key, user_type, lead_type, demand, motive, offer): (String, String, String, i64, i64, String)| {
    ...
    conn.exec_map(sql, (&run_id, keyword.clone(), keyword.clone(), keyword, req.min_value(), page_size, offset), |(user_key, user_type, lead_type, demand, motive, offer): (String, String, String, i64, i64, String)| {
        let label = user_key.clone();
