...
#[tauri::command]
pub fn analytics_get_lead_evidence_page(req: DashboardRequest) -> Result<AnalyticsQueryPage, String> {
    ...
    if let Ok(ads_detail) = batch_tables::resolve_table(
        &req.settings,
        &req.import_batch_id,
        ...
        let ads_count: Option<i64> = conn
            .exec_first(
                format!(
                    "SELECT CAST(COUNT(*) AS SIGNED) FROM `{ads_detail}` WHERE analysis_run_id=?"
                ),
                (&run_id,),
            )
            .unwrap_or(Some(0));
        if ads_count.unwrap_or(0) > 0 {
            ...
            let cards = conn.exec_map(sql, (&run_id, access_type.clone(), access_type.clone(), keyword.clone(), keyword.clone(), keyword.clone(), keyword, req.min_value(), fetch_size, offset), |(user_key, user_type, lead_type, demand, motive, offer): (String, String, String, i64, i64, String)| {
            ...
            return Ok(AnalyticsQueryPage::new(cards, &req, 200, 500, "ads_lead_evidence_detail"));
        }
    }
    ...
    let cards = conn.exec_map(sql, (&run_id, access_type.clone(), access_type, keyword.clone(), keyword.clone(), keyword.clone(), keyword, req.min_value(), fetch_size, offset), |(user_key, user_type, lead_type, demand, motive, offer): (String, String, String, i64, i64, String)| {
        let label = user_key.clone();
    ...
    Ok(AnalyticsQueryPage::new(cards, &req, 200, 500, "ads_migration_lead_user"))
}
