...
const S: &str = include_str!("../../database/sql/dws_to_ads/003f_analytics_network_hotspot.sql");

#[tauri::command]
pub fn ads_net(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    Ok(ack(format!("ads_net ok: analysis_run_id={run}; affected_rows={n}")))
}
