...
const S: &str = include_str!("../../database/sql/dws_to_ads/003c_analytics_hourly_trend.sql");

#[tauri::command]
pub fn ads_hour(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    Ok(ack(format!("ads_hour ok: analysis_run_id={run}; affected_rows={n}")))
}
