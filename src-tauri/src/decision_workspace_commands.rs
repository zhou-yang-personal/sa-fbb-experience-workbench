...
pub(crate) fn materialize_opportunities(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    let hourly_source = batch_tables::resolve_table(&req.settings, &req.import_batch_id, "dws_user_app_hourly_experience_v2")?;
    let mut conn = db::conn(&req.settings)?;
    ...
    if !batch_tables::table_has_analysis_run(&mut conn, &hourly_source, &run_id)? {
        return Err(format!("opportunity hourly source not ready for analysis_run_id={run_id}"));
    }
    ...
    let insert_sql = format!(r#"
        INSERT INTO ads_opportunity_user_v3
          (analysis_run_id,import_batch_id,user_key,opportunity_type,opportunity_level,
           evidence_value,evidence_unit,evidence_summary,data_limitation_code,rule_profile_version)
        WITH rules AS (
          SELECT p.* FROM meta_decision_rule_profile p
          JOIN meta_analysis_run_decision_binding b
            ON b.rule_profile_id=p.rule_profile_id AND b.rule_profile_version=p.version
          ...
          SELECT p.user_key,p.app_name,SUM(p.observation_rows) observations,MAX(a.active_days) active_days,
                 ROW_NUMBER() OVER(PARTITION BY p.user_key ORDER BY SUM(p.observation_rows) DESC,p.app_name) rn
          FROM `{source}` p
          JOIN (SELECT user_key,app_name,COUNT(DISTINCT stat_date) active_days FROM `{hourly_source}` WHERE analysis_run_id=? GROUP BY user_key,app_name) a ON a.user_key=p.user_key AND a.app_name=p.app_name
          WHERE p.analysis_run_id=? GROUP BY p.user_key,p.app_name
        )
        SELECT ?,?,u.user_key,'MIGRATION',
               CASE WHEN u.traffic_gb/NULLIF(u.active_days,0)>=r.heavy_traffic_gb THEN 'HIGH' ELSE 'STANDARD' END,
               u.traffic_gb/NULLIF(u.active_days,0),'GB/day',
               CONCAT('Cable; active_days=',u.active_days,'; daily traffic=',ROUND(u.traffic_gb/NULLIF(u.active_days,0),2),' GB; daily duration=',ROUND(u.usage_hours/NULLIF(u.active_days,0),2),' h'),
               NULL,r.version
        FROM users u CROSS JOIN rules r
        WHERE u.user_type='CABLE' AND u.active_days>=r.opportunity_min_active_days
          AND u.observations>=r.opportunity_min_observations
          AND (u.traffic_gb/NULLIF(u.active_days,0)>=r.migration_min_traffic_gb
            OR u.usage_hours/NULLIF(u.active_days,0)>=r.heavy_usage_hours
            OR u.observations/NULLIF(u.active_days,0)>=r.app_bundle_min_observations)
        UNION ALL
        SELECT ?,?,u.user_key,'SPEED_UPGRADE',
               CASE WHEN u.traffic_gb/NULLIF(u.active_days,0)>=r.heavy_traffic_gb THEN 'HIGH' ELSE 'STANDARD' END,
               u.traffic_gb/NULLIF(u.active_days,0),'GB/day',
               CONCAT('matched demand conditions; daily traffic=',ROUND(u.traffic_gb/NULLIF(u.active_days,0),2),' GB; daily duration=',ROUND(u.usage_hours/NULLIF(u.active_days,0),2),' h; effective rate=',COALESCE(ROUND(u.effective_mbps,2),'NA'),' Mbps'),
               NULL,r.version
        FROM users u CROSS JOIN rules r
        WHERE u.active_days>=r.opportunity_min_active_days
          AND u.observations>=r.opportunity_min_observations
          AND u.effective_mbps IS NOT NULL AND u.effective_mbps<=r.speed_upgrade_max_effective_mbps
          AND ((u.traffic_gb/NULLIF(u.active_days,0)>=r.speed_upgrade_min_traffic_gb)
        ...
        SELECT ?,?,u.user_key,'APP_BUNDLE',
               ...
               CONCAT('frequent app=',b.app_name,'; active_days=',b.active_days,'; daily observations=',ROUND(b.observations/NULLIF(b.active_days,0),2)),
               NULL,r.version
        FROM users u JOIN bundles b ON b.user_key=u.user_key AND b.rn=1 CROSS JOIN rules r
