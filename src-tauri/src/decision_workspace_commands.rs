...
fn decode_rule(row: mysql::Row) -> Result<DecisionRuleProfileRow, String> {
    Ok(DecisionRuleProfileRow {
        ...
        distribution_thresholds: row
            .get::<Option<String>, _>(33)
            .flatten()
            .and_then(|value| serde_json::from_str(&value).ok())
            .unwrap_or_else(default_distribution_thresholds),
        notes: row.get(34),
...
fn distribution_thresholds_for_run(
    conn: &mut mysql::PooledConn,
    run_id: &str,
) -> Result<DistributionThresholds, String> {
    ...
    let value = json
        ...
        .unwrap_or_else(default_distribution_thresholds);
    let array = |key: &str, defaults: &[f64]| {
        value
            .get(key)
            ...
            .filter(|items| items.len() == defaults.len())
            .unwrap_or_else(|| defaults.to_vec())
    };
    Ok(DistributionThresholds {
        traffic: array("traffic_daily_gb", &[1., 5., 15.]),
        duration: array("duration_daily_hours", &[1., 3., 6.]),
        peak: array("peak_daily_hours", &[0.5, 1.5, 3.]),
        observations: array("observations_daily", &[5., 20., 50.]),
        rate: array("rate_mbps", &[1., 5., 20., 50.]),
        rtt: array("rtt_ms", &[30., 60., 100., 200.]),
        loss: array("loss_pct", &[0.5, 1., 3.]),
    })
}
...
#[tauri::command]
pub fn decision_rule_list(
    settings: crate::models::MySqlSettings,
) -> Result<Vec<DecisionRuleProfileRow>, String> {
    let mut conn = db::conn(&settings)?;
    let rows = conn
        .query_iter(format!("{RULE_SELECT} ORDER BY version DESC"))
        .map_err(|err| format!("failed to list decision rules: {err}"))?
        .map(|row| {
            decode_rule(row.map_err(|err| format!("failed to decode decision rule: {err}"))?)
        })
        .collect();
    rows
}
...
#[tauri::command]
pub fn decision_rule_create_draft(
    settings: crate::models::MySqlSettings,
    ...
    if let Some(id) = existing {
        return Ok(ack(format!("existing decision rule draft reused: {id}")));
    }
...
#[tauri::command]
pub fn decision_rule_update(
    settings: crate::models::MySqlSettings,
    rule: DecisionRuleProfileRow,
) -> Result<CommandAck, String> {
    if rule.status != "draft" {
        return Err("only a draft decision rule can be edited".to_string());
    }
    if rule.minimum_user_observations < 1
        || rule.minimum_app_users < 1
        || rule.minimum_app_observations < 1
        || rule.sufficient_app_users < rule.minimum_app_users
        || rule.sufficient_app_observations < rule.minimum_app_observations
        || rule.opportunity_min_active_days < 1
        || rule.opportunity_min_observations < 1
        || rule.speed_upgrade_min_conditions < 1
        || rule.app_bundle_min_active_days < 1
    {
        return Err(
            "sample thresholds must be positive and sufficient must be at least limited"
                .to_string(),
        );
    }
    let rates = [
        rule.persistent_poor_rate_pct,
        rule.attention_app_poor_rate_pct,
        rule.attention_app_persistent_user_rate_pct,
        rule.problem_app_poor_rate_pct,
        rule.problem_app_persistent_user_rate_pct,
        rule.severe_app_poor_rate_pct,
        rule.severe_app_persistent_user_rate_pct,
        rule.severe_app_severe_user_rate_pct,
        rule.mesh_min_coverage_pct,
    ];
    if rates.iter().any(|value| !(0.0..=100.0).contains(value)) {
        return Err("rate thresholds must be between 0 and 100".to_string());
    }
    if rule.attention_app_poor_rate_pct > rule.problem_app_poor_rate_pct
        ...
        || rule.attention_app_persistent_user_rate_pct > rule.problem_app_persistent_user_rate_pct
        || rule.problem_app_persistent_user_rate_pct > rule.severe_app_persistent_user_rate_pct
    {
        return Err("App thresholds must increase from attention to problem to severe".to_string());
    }
    if !(0..=23).contains(&rule.peak_hour_start) || !(0..=23).contains(&rule.peak_hour_end) {
        return Err("peak hours must be between 0 and 23".to_string());
    }
    if rule.mesh_min_rtt_delta_ms < 0.0 || rule.mesh_min_loss_delta_pct < 0.0 {
        return Err("mesh evidence deltas cannot be negative".to_string());
    }
    for key in [
        "traffic_daily_gb",
        "duration_daily_hours",
        "peak_daily_hours",
        "observations_daily",
        "rate_mbps",
        "rtt_ms",
        ...
        let values = rule
            ...
            .ok_or_else(|| format!("distribution threshold {key} must be an array"))?;
        if values.is_empty()
            || values
                .iter()
                .any(|value| value.as_f64().is_none_or(|number| number < 0.0))
            || values.windows(2).any(|pair| {
                pair[0].as_f64().unwrap_or_default() >= pair[1].as_f64().unwrap_or_default()
            })
        {
            return Err(format!(
                "distribution threshold {key} must contain increasing non-negative numbers"
            ));
        }
    }
    let distribution_thresholds = serde_json::to_string(&rule.distribution_thresholds)
        .map_err(|err| format!("failed to encode distribution thresholds: {err}"))?;
    let mut conn = db::conn(&settings)?;
    ...
    Ok(ack(format!(
        "decision rule draft saved: {}",
        rule.rule_profile_id
    )))
}

#[tauri::command]
pub fn decision_rule_publish(
    settings: crate::models::MySqlSettings,
    ...
    if conn.affected_rows() != 1 {
        return Err("decision rule draft not found or already published".to_string());
    }
...
fn row_optional_string(row: &mysql::Row, index: usize) -> Option<String> {
    row.get_opt::<Option<String>, _>(index)
        .and_then(Result::ok)
        .flatten()
}
...
fn require_decision_subtasks_ready(
    conn: &mut mysql::PooledConn,
    analysis_run_id: &str,
    subtasks: &[&str],
) -> Result<(), String> {
    let active_pipeline: Option<String> = conn
        .exec_first(
            "SELECT pipeline_run_id FROM meta_pipeline_run WHERE analysis_run_id=? AND status IN ('pending','running') ORDER BY updated_at DESC LIMIT 1",
            (analysis_run_id,),
        )
        .map_err(|err| format!("failed to inspect decision publication readiness: {err}"))?;
    if let Some(pipeline_run_id) = active_pipeline {
        return Err(format!(
            "DECISION_DATA_NOT_READY: analysis_run_id={analysis_run_id} is being rebuilt by pipeline_run_id={pipeline_run_id}; wait for a successful publication"
        ));
    }
    for subtask in subtasks {
        let expected_implementation =
            crate::import_pipeline_commands::aggregation_implementation_version(subtask);
        let expected_source = crate::import_pipeline_commands::aggregation_source_version(subtask);
        let checkpoint: Option<(String, String, Option<String>)> = conn
            .exec_first(
            ...
            .map_err(|err| format!("failed to inspect decision checkpoint {subtask}: {err}"))?;
        let ready =
            checkpoint
                .as_ref()
                .is_some_and(|(status, implementation_version, source_version)| {
                    crate::import_pipeline_commands::aggregation_checkpoint_is_reusable(
                        status,
        ...
        if !ready {
            return Err(format!(
                "DECISION_DATA_NOT_READY: subtask={subtask} has no successful checkpoint for implementation={expected_implementation}, source={expected_source}; resume from DWS/ADS in the data job center"
            ));
        }
    }
    Ok(())
...
fn require_rule_binding(conn: &mut mysql::PooledConn, analysis_run_id: &str) -> Result<(), String> {
...
pub fn decision_get_metric_panorama(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_metric_panorama", || {
        ...
        require_decision_subtasks_ready(&mut conn, &run_id, &["complete_dws"])?;
        ...
        let values = [
            ...
            ("视频有效下载速率", fmt_optional(row_f64(&row, 8), 2), format!("metric=effective_download_rate, unit=Mbps, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 8).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("平均下载速率", fmt_optional(row_f64(&row, 14), 2), format!("metric=average_download_rate, unit=Mbps, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 14).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("平均吞吐带宽", fmt_optional(row_f64(&row, 15), 2), format!("metric=throughput, unit=Mbps, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 15).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("建连成功率", fmt_optional(row_f64(&row, 16), 2), format!("metric=connection_success, unit=percent, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 16).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("建连时延", fmt_optional(row_f64(&row, 17), 2), format!("metric=connection_delay, unit=ms, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 17).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ...
            ("用户侧时延", fmt_optional(row_f64(&row, 9), 2), format!("metric=subscriber_rtt, unit=ms, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 9).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("网络侧时延", fmt_optional(row_f64(&row, 10), 2), format!("metric=network_rtt, unit=ms, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 10).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("用户侧丢包", fmt_optional(row_f64(&row, 11), 4), format!("metric=user_loss, unit=percent, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 11).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("网络侧丢包", fmt_optional(row_f64(&row, 12), 4), format!("metric=network_loss, unit=percent, sample_size={valid}, availability={}, limitation=当前发布版本未生成该指标或有效样本数为 0", if row_f64(&row, 12).is_some() { "AVAILABLE" } else { "UNAVAILABLE" })),
            ("上行速率", "—".to_string(), "metric=upstream_rate, unit=Mbps, availability=UNAVAILABLE, limitation=当前已识别并映射的字段中没有可确认的上行吞吐速率；请核对原始 CSV 表头及供应商字段字典".to_string()),
        ];
        Ok(values
            .into_iter()
            .map(|(label, value, hint)| MetricCard {
...
pub fn decision_get_app_panorama(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_app_panorama", || {
        let mut conn = db::conn(&req.settings)?;
        let run_id = req.run_id();
        require_decision_subtasks_ready(&mut conn, &run_id, &["app_rank"])?;
        let table = source_table(&req)?;
        ensure_run_source(&req, &table)?;
        require_rule_binding(&mut conn, &run_id)?;
...
pub fn decision_get_user_distributions(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_user_distributions", || {
        ...
        let [traffic0, traffic1, traffic2] = thresholds.traffic[..] else {
            unreachable!()
        };
        let [duration0, duration1, duration2] = thresholds.duration[..] else {
            unreachable!()
        };
        let [peak0, peak1, peak2] = thresholds.peak[..] else {
            unreachable!()
        };
        let [obs0, obs1, obs2] = thresholds.observations[..] else {
            unreachable!()
        };
        let [rate0, rate1, rate2, rate3] = thresholds.rate[..] else {
            unreachable!()
        };
        let [rtt0, rtt1, rtt2, rtt3] = thresholds.rtt[..] else {
            unreachable!()
        };
        let [loss0, loss1, loss2] = thresholds.loss[..] else {
            unreachable!()
        };
        let keyword = req.keyword_like();
        let sql = format!(
        ...
        let rows = conn
            .exec_iter(sql, (&run_id, &run_id, &run_id, keyword.clone(), keyword))
            .map_err(|err| format!("failed to query user distributions: {err}"))?;
        rows.map(|row| {
...
pub fn decision_get_quality_overview(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_quality_overview", || {
        let mut conn = db::conn(&req.settings)?;
        let run = req.run_id();
        require_decision_subtasks_ready(&mut conn, &run, &["complete_dws"])?;
        let table = source_table(&req)?;
        ensure_run_source(&req, &table)?;
        let sql = format!(
        ...
        let rows = conn
            .exec_iter(sql, (&run, &run, &run, &run))
            .map_err(|err| format!("failed to query quality overview: {err}"))?;
        rows.map(|row| {
...
pub fn decision_get_access_compare(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_access_compare", || {
        let mut conn = db::conn(&req.settings)?;
        require_decision_subtasks_ready(&mut conn, &req.run_id(), &["access_overview"])?;
...
pub fn decision_get_access_hourly(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_access_hourly", || {
        let mut conn = db::conn(&req.settings)?;
        let run_id = req.run_id();
        require_decision_subtasks_ready(&mut conn, &run_id, &["access_hourly"])?;
        require_rule_binding(&mut conn, &run_id)?;
        let (peak_start, peak_end): (i64, i64) = conn.exec_first("SELECT CAST(p.peak_hour_start AS SIGNED),CAST(p.peak_hour_end AS SIGNED) FROM meta_decision_rule_profile p JOIN meta_analysis_run_decision_binding b ON b.rule_profile_id=p.rule_profile_id AND b.rule_profile_version=p.version WHERE b.analysis_run_id=?", (&run_id,)).map_err(|err| format!("failed to query bound peak hours: {err}"))?.ok_or_else(|| format!("decision rule binding is not ready for analysis_run_id={run_id}"))?;
        let rows = conn.exec_iter("SELECT user_type,CAST(hour_of_day AS SIGNED),CAST(observed_users AS SIGNED),CAST(observation_rows AS SIGNED),CAST(valid_obs_rows AS SIGNED),CAST(poor_obs_rows AS SIGNED),CAST(total_download_gb AS DOUBLE),CAST(total_effective_duration_hours AS DOUBLE),CAST(avg_effective_download_mbps AS DOUBLE),CAST(avg_download_mbps AS DOUBLE),CAST(avg_subscriber_rtt_ms AS DOUBLE),CAST(avg_network_rtt_ms AS DOUBLE),CAST(avg_user_loss_pct AS DOUBLE),CAST(avg_network_loss_pct AS DOUBLE) FROM ads_access_hourly_v2 WHERE analysis_run_id=? AND user_type IN ('CABLE','FTTH') ORDER BY hour_of_day,FIELD(user_type,'CABLE','FTTH')", (&run_id,)).map_err(|err| format!("failed to query access hourly: {err}"))?;
        rows.map(|row| {
            ...
            Ok(MetricCard { label: format!("{hour:02}:00 {access}"), value: users.to_string(), hint: format!("dimension=ACCESS_HOURLY, access_type={access}, hour={hour}, peak_hour_start={peak_start}, peak_hour_end={peak_end}, users={users}, observations={}, valid_obs_rows={valid}, poor_obs_rows={poor}, poor_observation_rate_pct={}, traffic_gb={}, duration_hours={}, effective_download_mbps={}, average_download_mbps={}, subscriber_rtt_ms={}, network_rtt_ms={}, user_loss_pct={}, network_loss_pct={}", row_i64(&row,3), fmt_optional(rate(poor,valid),4), fmt_optional(row_f64(&row,6),4), fmt_optional(row_f64(&row,7),4), fmt_optional(row_f64(&row,8),4), fmt_optional(row_f64(&row,9),4), fmt_optional(row_f64(&row,10),4), fmt_optional(row_f64(&row,11),4), fmt_optional(row_f64(&row,12),6), fmt_optional(row_f64(&row,13),6)) })
        }).collect()
    })
}
...
pub fn decision_get_panorama_hourly(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_panorama_hourly", || {
        ...
        require_rule_binding(&mut conn, &run_id)?;
        let (peak_start, peak_end): (i64, i64) = conn.exec_first("SELECT CAST(p.peak_hour_start AS SIGNED),CAST(p.peak_hour_end AS SIGNED) FROM meta_decision_rule_profile p JOIN meta_analysis_run_decision_binding b ON b.rule_profile_id=p.rule_profile_id AND b.rule_profile_version=p.version WHERE b.analysis_run_id=?", (&run_id,)).map_err(|err| format!("failed to query bound peak hours: {err}"))?.ok_or_else(|| format!("decision rule binding is not ready for analysis_run_id={run_id}"))?;
        let rows=conn.exec_iter("SELECT CAST(hour_of_day AS SIGNED),CAST(observed_users AS SIGNED),CAST(observation_rows AS SIGNED),CAST(valid_obs_rows AS SIGNED),CAST(poor_obs_rows AS SIGNED),CAST(total_download_gb AS DOUBLE),CAST(total_effective_duration_hours AS DOUBLE),CAST(avg_effective_download_mbps AS DOUBLE),CAST(avg_download_mbps AS DOUBLE),CAST(avg_subscriber_rtt_ms AS DOUBLE),CAST(avg_network_rtt_ms AS DOUBLE),CAST(avg_user_loss_pct AS DOUBLE),CAST(avg_network_loss_pct AS DOUBLE) FROM ads_access_hourly_v2 WHERE analysis_run_id=? AND user_type='ALL' ORDER BY hour_of_day",(&run_id,)).map_err(|err| format!("failed to query panorama hourly: {err}"))?;
        rows.map(|row| { let row=row.map_err(|err| format!("failed to decode panorama hourly: {err}"))?; let hour=row_i64(&row,0); let users=row_i64(&row,1); let valid=row_i64(&row,3); let poor=row_i64(&row,4); Ok(MetricCard { label: format!("{hour:02}:00"),value:users.to_string(),hint:format!("dimension=PANORAMA_HOURLY, hour={hour}, peak_hour_start={peak_start}, peak_hour_end={peak_end}, users={users}, observations={}, valid_obs_rows={valid}, poor_obs_rows={poor}, poor_observation_rate_pct={}, traffic_gb={}, duration_hours={}, effective_download_mbps={}, average_download_mbps={}, subscriber_rtt_ms={}, network_rtt_ms={}, user_loss_pct={}, network_loss_pct={}",row_i64(&row,2),fmt_optional(rate(poor,valid),4),fmt_optional(row_f64(&row,5),4),fmt_optional(row_f64(&row,6),4),fmt_optional(row_f64(&row,7),4),fmt_optional(row_f64(&row,8),4),fmt_optional(row_f64(&row,9),4),fmt_optional(row_f64(&row,10),4),fmt_optional(row_f64(&row,11),6),fmt_optional(row_f64(&row,12),6)) }) }).collect()
    })
}
...
pub fn decision_get_access_user_bands(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_access_user_bands", || {
        let mut conn = db::conn(&req.settings)?;
        require_decision_subtasks_ready(&mut conn, &req.run_id(), &["access_bands"])?;
...
pub fn decision_get_access_apps(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_access_apps", || {
        let mut conn = db::conn(&req.settings)?;
        require_decision_subtasks_ready(&mut conn, &req.run_id(), &["complete_dws"])?;
        ...
        let rows = conn
            .exec_iter(sql, (req.run_id(), limit))
            .map_err(|err| format!("failed to query access App compare: {err}"))?;
...
pub(crate) fn materialize_access_user_core(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    let period = batch_tables::resolve_table(
        &req.settings,
        &req.import_batch_id,
        "dws_user_app_period_experience_v2",
    )?;
    let daily = batch_tables::resolve_table(
        &req.settings,
        &req.import_batch_id,
    ...
    if !batch_tables::table_has_analysis_run(&mut conn, &period, &run_id)? {
        return Err(format!(
            "access period source not ready for analysis_run_id={run_id}"
        ));
    }
    ensure_opportunity_rules(&mut conn, &req, &run_id)?;
    conn.exec_drop(
        "DELETE FROM dws_user_access_period_v2 WHERE analysis_run_id=?",
        (&run_id,),
    )
    .map_err(|err| format!("failed to reset access user core: {err}"))?;
    let sql = format!(
        r#"
    ...
    conn.exec_drop(sql, (&run_id, &run_id, &req.import_batch_id))
        .map_err(|err| format!("failed to materialize access user core: {err}"))?;
    Ok(ack(format!(
        "access user core ready: analysis_run_id={run_id}, affected_rows={}",
        conn.affected_rows()
    )))
}
...
pub(crate) fn materialize_access_overview(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    let mut conn = db::conn(&req.settings)?;
    conn.exec_drop(
        "DELETE FROM ads_access_overview_v2 WHERE analysis_run_id=?",
        (&run_id,),
    )
    .map_err(|err| format!("failed to reset access overview: {err}"))?;
    ...
    Ok(ack(format!(
        "access overview ready: analysis_run_id={run_id}, affected_rows={}",
        conn.affected_rows()
    )))
}
...
pub(crate) fn materialize_access_hourly(req: EtlRequest) -> Result<CommandAck, String> {
    crate::migrations::ensure_decision_workspace_schema(&req.settings)?;
    let run_id = opportunity_run_id(&req);
    let hourly = batch_tables::resolve_table(
        &req.settings,
        &req.import_batch_id,
        "dws_user_app_hourly_experience_v2",
    )?;
    let mut conn = db::conn(&req.settings)?;
    if !batch_tables::table_has_analysis_run(&mut conn, &hourly, &run_id)? {
        return Err(format!(
            "access hourly source not ready for analysis_run_id={run_id}"
        ));
    }
    conn.exec_drop(
        "DELETE FROM ads_access_hourly_v2 WHERE analysis_run_id=?",
        (&run_id,),
    )
    .map_err(|err| format!("failed to reset access hourly: {err}"))?;
    let sql=format!("INSERT INTO ads_access_hourly_v2 (analysis_run_id,import_batch_id,user_type,hour_of_day,observed_users,observation_rows,valid_obs_rows,poor_obs_rows,total_download_gb,total_effective_duration_hours,avg_effective_download_mbps,avg_download_mbps,avg_subscriber_rtt_ms,avg_network_rtt_ms,avg_user_loss_pct,avg_network_loss_pct,effective_download_mbps_sum,effective_download_mbps_count,avg_download_mbps_sum,avg_download_mbps_count,subscriber_rtt_sum,subscriber_rtt_count,network_rtt_sum,network_rtt_count,user_loss_sum,user_loss_count,network_loss_sum,network_loss_count) SELECT analysis_run_id,MAX(import_batch_id),user_type,hour_of_day,COUNT(DISTINCT user_key),SUM(observation_rows),SUM(valid_obs_rows),SUM(poor_obs_rows),SUM(total_download_gb),SUM(total_effective_duration_hours),SUM(effective_download_mbps_sum)/NULLIF(SUM(effective_download_mbps_count),0),SUM(avg_download_mbps_sum)/NULLIF(SUM(avg_download_mbps_count),0),SUM(subscriber_rtt_sum)/NULLIF(SUM(subscriber_rtt_count),0),SUM(network_rtt_sum)/NULLIF(SUM(network_rtt_count),0),SUM(user_loss_sum)/NULLIF(SUM(user_loss_count),0),SUM(network_loss_sum)/NULLIF(SUM(network_loss_count),0),SUM(effective_download_mbps_sum),SUM(effective_download_mbps_count),SUM(avg_download_mbps_sum),SUM(avg_download_mbps_count),SUM(subscriber_rtt_sum),SUM(subscriber_rtt_count),SUM(network_rtt_sum),SUM(network_rtt_count),SUM(user_loss_sum),SUM(user_loss_count),SUM(network_loss_sum),SUM(network_loss_count) FROM `{hourly}` WHERE analysis_run_id=? GROUP BY analysis_run_id,user_type,hour_of_day");
    conn.exec_drop(sql, (&run_id,))
        .map_err(|err| format!("failed to materialize access hourly: {err}"))?;
    conn.exec_drop("INSERT INTO ads_access_hourly_v2 (analysis_run_id,import_batch_id,user_type,hour_of_day,observed_users,observation_rows,valid_obs_rows,poor_obs_rows,total_download_gb,total_effective_duration_hours,avg_effective_download_mbps,avg_download_mbps,avg_subscriber_rtt_ms,avg_network_rtt_ms,avg_user_loss_pct,avg_network_loss_pct,effective_download_mbps_sum,effective_download_mbps_count,avg_download_mbps_sum,avg_download_mbps_count,subscriber_rtt_sum,subscriber_rtt_count,network_rtt_sum,network_rtt_count,user_loss_sum,user_loss_count,network_loss_sum,network_loss_count) SELECT analysis_run_id,MAX(import_batch_id),'ALL',hour_of_day,SUM(observed_users),SUM(observation_rows),SUM(valid_obs_rows),SUM(poor_obs_rows),SUM(total_download_gb),SUM(total_effective_duration_hours),SUM(effective_download_mbps_sum)/NULLIF(SUM(effective_download_mbps_count),0),SUM(avg_download_mbps_sum)/NULLIF(SUM(avg_download_mbps_count),0),SUM(subscriber_rtt_sum)/NULLIF(SUM(subscriber_rtt_count),0),SUM(network_rtt_sum)/NULLIF(SUM(network_rtt_count),0),SUM(user_loss_sum)/NULLIF(SUM(user_loss_count),0),SUM(network_loss_sum)/NULLIF(SUM(network_loss_count),0),SUM(effective_download_mbps_sum),SUM(effective_download_mbps_count),SUM(avg_download_mbps_sum),SUM(avg_download_mbps_count),SUM(subscriber_rtt_sum),SUM(subscriber_rtt_count),SUM(network_rtt_sum),SUM(network_rtt_count),SUM(user_loss_sum),SUM(user_loss_count),SUM(network_loss_sum),SUM(network_loss_count) FROM ads_access_hourly_v2 WHERE analysis_run_id=? AND user_type<>'ALL' GROUP BY analysis_run_id,hour_of_day",(&run_id,)).map_err(|err| format!("failed to materialize panorama hourly rollup: {err}"))?;
    Ok(ack(format!(
        "access hourly ready: analysis_run_id={run_id}, affected_rows={}",
        conn.affected_rows()
    )))
}
...
pub(crate) fn materialize_access_bands(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    let [rtt0, rtt1, rtt2, _] = thresholds.rtt[..] else {
        unreachable!()
    };
    let [loss0, loss1, loss2] = thresholds.loss[..] else {
        unreachable!()
    ...
    conn.exec_drop(
        "DELETE FROM ads_access_user_band_v2 WHERE analysis_run_id=?",
        (&run_id,),
    )
    .map_err(|err| format!("failed to reset access bands: {err}"))?;
    ...
    conn.exec_drop(sql, (&run_id,))
        .map_err(|err| format!("failed to materialize access bands: {err}"))?;
    Ok(ack(format!(
        "access user bands ready: analysis_run_id={run_id}, affected_rows={}",
        conn.affected_rows()
    )))
}
...
pub(crate) fn materialize_opportunity_features(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    if !batch_tables::table_has_analysis_run(&mut conn, &period, &run_id)? {
        return Err(format!(
            "opportunity period source not ready for analysis_run_id={run_id}"
        ));
    }
    ensure_opportunity_rules(&mut conn, &req, &run_id)?;
    conn.exec_drop(
        "DELETE FROM dws_user_opportunity_feature_v2 WHERE analysis_run_id=?",
        (&run_id,),
    )
    .map_err(|err| format!("failed to reset opportunity features: {err}"))?;
    let sql = format!(
        r#"
    ...
    .map_err(|err| format!("failed to materialize opportunity features: {err}"))?;
    Ok(ack(format!(
        "opportunity features ready: analysis_run_id={run_id}, affected_rows={}",
        conn.affected_rows()
    )))
}

fn reset_staging_type(
    conn: &mut mysql::PooledConn,
    run_id: &str,
    kind: &str,
) -> Result<(), String> {
    conn.exec_drop(
        "DELETE FROM stg_opportunity_user_v3 WHERE analysis_run_id=? AND opportunity_type=?",
        (run_id, kind),
    )
    .map_err(|err| format!("failed to reset {kind} opportunity staging: {err}"))
}
...
fn materialize_opportunity_type(req: EtlRequest, kind: &str) -> Result<CommandAck, String> {
    ...
    let select = OPPORTUNITY_FEATURE_COLUMNS
        .replace("TYPE_TOKEN", &format!("'{kind}'"))
        .replace("LEVEL_TOKEN", level)
        .replace("EVIDENCE_VALUE", evidence_value)
        .replace("EVIDENCE_UNIT", evidence_unit)
        .replace("EVIDENCE_SUMMARY", evidence_summary);
    let coverage = if kind == "MESH_AP" {
    ...
    if kind == "MESH_AP" {
        conn.exec_drop(sql, (&run_id, &run_id))
            .map_err(|err| format!("failed to materialize {kind} opportunities: {err}"))?;
    } else {
        conn.exec_drop(sql, (&run_id,))
            .map_err(|err| format!("failed to materialize {kind} opportunities: {err}"))?;
    }
    Ok(ack(format!(
        "{kind} opportunities staged: analysis_run_id={run_id}, affected_rows={}",
        conn.affected_rows()
    )))
}

pub(crate) fn materialize_opportunity_migration(req: EtlRequest) -> Result<CommandAck, String> {
    materialize_opportunity_type(req, "MIGRATION")
}
pub(crate) fn materialize_opportunity_speed_upgrade(req: EtlRequest) -> Result<CommandAck, String> {
    materialize_opportunity_type(req, "SPEED_UPGRADE")
}
pub(crate) fn materialize_opportunity_mesh(req: EtlRequest) -> Result<CommandAck, String> {
    materialize_opportunity_type(req, "MESH_AP")
}
pub(crate) fn materialize_opportunity_app_bundle(req: EtlRequest) -> Result<CommandAck, String> {
    materialize_opportunity_type(req, "APP_BUNDLE")
}
...
pub(crate) fn publish_opportunities(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    let mut tx = conn
        ...
        .map_err(|err| format!("failed to start opportunity publish transaction: {err}"))?;
    tx.exec_drop(
        "DELETE FROM ads_opportunity_summary_v3 WHERE analysis_run_id=?",
        (&run_id,),
    )
    .map_err(|err| format!("failed to reset published opportunity summary: {err}"))?;
    tx.exec_drop(
        "DELETE FROM ads_opportunity_user_v3 WHERE analysis_run_id=?",
        (&run_id,),
    )
    .map_err(|err| format!("failed to reset published opportunity users: {err}"))?;
    ...
    tx.commit()
        .map_err(|err| format!("failed to commit opportunity publication: {err}"))?;
    Ok(ack(format!(
        "opportunities atomically published: analysis_run_id={run_id}"
    )))
}
...
pub fn decision_get_opportunities(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_opportunities", || {
        let mut conn = db::conn(&req.settings)?;
        require_decision_subtasks_ready(&mut conn, &req.run_id(), &["opportunity_publish"])?;
...
#[tauri::command]
pub fn decision_get_opportunity_candidates(
    req: DashboardRequest,
) -> Result<OpportunityCandidatePage, String> {
    crate::command_guard::run("decision_get_opportunity_candidates", || {
        ...
        let keyword = req.keyword_like();
        let mut conn = db::conn(&req.settings)?;
        require_decision_subtasks_ready(&mut conn, &run_id, &["opportunity_publish"])?;
        ...
        let rows = rows
            .map(|row| {
                let row =
                    row.map_err(|err| format!("failed to decode opportunity candidate: {err}"))?;
                Ok(OpportunityCandidateRow {
                    user_key: row_string(&row, 0, "UNKNOWN_USER"),
                    opportunity_type: row_string(&row, 1, "UNKNOWN"),
                    opportunity_level: row_string(&row, 2, "UNKNOWN"),
                    user_type: row_string(&row, 3, "UNKNOWN"),
                    active_days: row_i64(&row, 4),
                    observation_rows: row_i64(&row, 5),
                    total_download_gb: row_f64(&row, 6).unwrap_or_default(),
                    total_effective_duration_hours: row_f64(&row, 7).unwrap_or_default(),
                    avg_effective_download_mbps: row_f64(&row, 8),
                    avg_wifi_delay_ms: row_f64(&row, 9),
                    ...
                    primary_app: row_optional_string(&row, 14),
                    primary_app_active_days: row_i64(&row, 15),
                    primary_app_observations: row_i64(&row, 16),
                    evidence_value: row_f64(&row, 17),
                    evidence_unit: row_optional_string(&row, 18),
                    evidence_summary: row_string(&row, 19, ""),
                    data_limitation_code: row_optional_string(&row, 20),
                    rule_profile_version: row_i64(&row, 21),
                })
            })
            .collect::<Result<Vec<_>, String>>()?;
        Ok(OpportunityCandidatePage {
            rows,
...
pub fn decision_export_opportunity_candidates_csv(
    ...
    crate::command_guard::run("decision_export_opportunity_candidates_csv", || {
        ...
        let mut conn = db::conn(&req.settings)?;
        require_decision_subtasks_ready(&mut conn, &req.analysis_run_id, &["opportunity_publish"])?;
        ...
        writer
            .write_record([
                ...
                "avg_network_rtt_ms",
                "avg_user_loss_pct",
                "avg_network_loss_pct",
                "primary_app",
                "primary_app_active_days",
                "primary_app_observations",
                "evidence_value",
                "evidence_unit",
                "evidence_summary",
                "data_limitation_code",
                "rule_profile_version",
            ])
        ...
        let rows = conn
            .exec_iter(query, Params::Positional(values))
            .map_err(|err| format!("failed to query opportunity candidates for export: {err}"))?;
        let mut exported_rows = 0_u64;
        for row in rows {
            let row =
                row.map_err(|err| format!("failed to decode opportunity export row: {err}"))?;
            let active_days = row_i64(&row, 4);
            let observation_rows = row_i64(&row, 5);
            let total_download_gb = row_f64(&row, 6).unwrap_or_default();
            let total_effective_duration_hours = row_f64(&row, 7).unwrap_or_default();
            let primary_app_active_days = row_i64(&row, 15);
            let primary_app_observations = row_i64(&row, 16);
            let rule_profile_version = row_i64(&row, 21);
            writer
                .write_record([
                    row_string(&row, 0, "UNKNOWN_USER"),
                    row_string(&row, 1, "UNKNOWN"),
                    row_string(&row, 2, "UNKNOWN"),
                    row_string(&row, 3, "UNKNOWN"),
                    active_days.to_string(),
                    observation_rows.to_string(),
                    total_download_gb.to_string(),
                    total_effective_duration_hours.to_string(),
                    export_number(row_f64(&row, 8)),
                    export_number(row_f64(&row, 9)),
                    export_number(row_f64(&row, 10)),
                    export_number(row_f64(&row, 11)),
                    export_number(row_f64(&row, 12)),
                    export_number(row_f64(&row, 13)),
                    export_string(row_optional_string(&row, 14)),
                    primary_app_active_days.to_string(),
                    primary_app_observations.to_string(),
                    export_number(row_f64(&row, 17)),
                    export_string(row_optional_string(&row, 18)),
                    row_string(&row, 19, ""),
                    export_string(row_optional_string(&row, 20)),
                    rule_profile_version.to_string(),
                ])
        ...
        writer
            .flush()
            .map_err(|err| format!("failed to flush opportunity export: {err}"))?;
        Ok(ack(format!(
