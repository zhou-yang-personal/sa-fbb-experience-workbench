...
pub fn decision_get_access_hourly(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_access_hourly", || {
        let mut conn = db::conn(&req.settings)?;
        require_decision_subtasks_ready(&mut conn, &req.run_id(), &["access_hourly"])?;
        ...
        rows.map(|row| {
            ...
            Ok(MetricCard { label: format!("{hour:02}:00 {access}"), value: users.to_string(), hint: format!("dimension=ACCESS_HOURLY, access_type={access}, hour={hour}, users={users}, observations={}, valid_obs_rows={valid}, poor_obs_rows={poor}, poor_observation_rate_pct={}, traffic_gb={}, duration_hours={}, effective_download_mbps={}, average_download_mbps={}, subscriber_rtt_ms={}, network_rtt_ms={}, user_loss_pct={}, network_loss_pct={}", row_i64(&row,3), fmt_optional(rate(poor,valid),4), fmt_optional(row_f64(&row,6),4), fmt_optional(row_f64(&row,7),4), fmt_optional(row_f64(&row,8),4), fmt_optional(row_f64(&row,9),4), fmt_optional(row_f64(&row,10),4), fmt_optional(row_f64(&row,11),4), fmt_optional(row_f64(&row,12),6), fmt_optional(row_f64(&row,13),6)) })
        }).collect()
    })
}
...
pub fn decision_get_panorama_hourly(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    crate::command_guard::run("decision_get_panorama_hourly", || {
        ...
        rows.map(|row| { let row=row.map_err(|err| format!("failed to decode panorama hourly: {err}"))?; let hour=row_i64(&row,0); let users=row_i64(&row,1); let valid=row_i64(&row,3); let poor=row_i64(&row,4); Ok(MetricCard { label: format!("{hour:02}:00"),value:users.to_string(),hint:format!("dimension=PANORAMA_HOURLY, hour={hour}, users={users}, observations={}, valid_obs_rows={valid}, poor_obs_rows={poor}, poor_observation_rate_pct={}, traffic_gb={}, duration_hours={}, effective_download_mbps={}, average_download_mbps={}, subscriber_rtt_ms={}, network_rtt_ms={}, user_loss_pct={}, network_loss_pct={}",row_i64(&row,2),fmt_optional(rate(poor,valid),4),fmt_optional(row_f64(&row,5),4),fmt_optional(row_f64(&row,6),4),fmt_optional(row_f64(&row,7),4),fmt_optional(row_f64(&row,8),4),fmt_optional(row_f64(&row,9),4),fmt_optional(row_f64(&row,10),4),fmt_optional(row_f64(&row,11),6),fmt_optional(row_f64(&row,12),6)) }) }).collect()
    })
}
