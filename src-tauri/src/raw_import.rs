...
fn load_data(req: RawLoadRequest) -> Result<String, String> {
    ...
    conn.exec_drop(
        "UPDATE meta_import_batch SET status='running', started_at=NOW(), message='raw load_data started' WHERE import_batch_id=?",
        (&req.import_batch_id,),
    )
    .map_err(|err| format!("failed to mark batch running: {err}"))?;

    let sql = format!(
        "{load_keyword} DATA LOCAL INFILE '{path}' INTO TABLE {} CHARACTER SET utf8mb4 FIELDS TERMINATED BY ',' ENCLOSED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 LINES ({columns}) SET import_batch_id='{batch_id}', source_file_name='{source_name}', source_line_no=NULL",
        spec.table
    );
...
fn streaming_insert(req: RawLoadRequest) -> Result<String, String> {
    ...
    let file_name = source_file_name(&req.file_path);
    conn.exec_drop(
        "UPDATE meta_import_batch SET status='running', started_at=NOW(), message='streaming insert started' WHERE import_batch_id=?",
        (&req.import_batch_id,),
    )
    .map_err(|err| format!("failed to mark batch running: {err}"))?;
    ...
    let mut reader = csv::ReaderBuilder::new()
        ...
        .map_err(|err| format!("failed to open CSV for streaming insert: {err}"))?;
    let headers = reader
        .headers()
...
fn flush_rows(conn: &mut mysql::PooledConn, spec: RawSpec, rows: &[Vec<String>]) -> Result<(), String> {
    ...
    conn.query_drop(sql)
        .map_err(|err| format!("failed to insert streaming RAW rows: {err}"))
}

fn finalize_batch(conn: &mut mysql::PooledConn, batch_id: &str, rows: u64, message: &str) -> Result<(), String> {
    conn.exec_drop(
        "UPDATE meta_import_batch SET status='success', imported_rows=?, finished_at=NOW(), message=? WHERE import_batch_id=?",
