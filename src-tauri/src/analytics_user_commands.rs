...
#[tauri::command]
pub fn analytics_get_user_profiles(req: DashboardRequest) -> Result<AnalyticsQueryPage, String> {
    ...
    if let Ok(ads_table) = batch_tables::resolve_table(
        ...
        if ads_count.unwrap_or(0) > 0 {
            ...
            let cards = conn.exec_map(sql, (&run_id, access_type.clone(), access_type.clone(), keyword.clone(), keyword.clone(), keyword.clone(), keyword, req.min_value(), fetch_size, offset), |(user_key, user_type, days, gb, hours, vmos, mos, rtt, lead_type, demand, motive): (String, String, i64, f64, f64, f64, f64, f64, String, i64, i64)| {
            ...
            return Ok(AnalyticsQueryPage::new(cards, &req, 160, 500, "ads_user_experience_profile"));
        }
    }
    ...
    Ok(AnalyticsQueryPage::new(cards, &req, 160, 500, "dws_user_daily_profile"))
}
...
pub fn analytics_get_user_summary(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    ...
    let keyword = req.keyword_like();
    let sql = format!(
        "WITH filtered AS (SELECT * FROM `{table}` WHERE analysis_run_id=? AND (? IS NULL OR COALESCE(user_type,'UNKNOWN')=?) AND (? IS NULL OR user_key LIKE ? OR COALESCE(user_type,'UNKNOWN') LIKE ? OR COALESCE(lead_type,'NONE') LIKE ? OR COALESCE(bottleneck_side,'UNKNOWN') LIKE ?)) \
         SELECT summary_type, segment, user_type, CAST(COUNT(*) AS SIGNED) AS users, CAST(ROUND(AVG(demand_score),2) AS DOUBLE) AS avg_demand FROM (\
         SELECT 'demand_band' AS summary_type, CASE WHEN COALESCE(demand_score,0)>=60 THEN '60-100 高需求' WHEN COALESCE(demand_score,0)>=40 THEN '40-59 中需求' WHEN COALESCE(demand_score,0)>=20 THEN '20-39 轻需求' ELSE '0-19 低需求' END AS segment, COALESCE(user_type,'UNKNOWN') AS user_type, COALESCE(demand_score,0) AS demand_score FROM filtered \
         UNION ALL \
         SELECT 'traffic_band', CASE WHEN COALESCE(total_traffic_gb,0)>=20 THEN '20GB+ 高流量' WHEN COALESCE(total_traffic_gb,0)>=10 THEN '10-20GB' WHEN COALESCE(total_traffic_gb,0)>=1 THEN '1-10GB' ELSE '<1GB' END, COALESCE(user_type,'UNKNOWN'), COALESCE(demand_score,0) FROM filtered \
         UNION ALL \
         SELECT 'bottleneck_side', COALESCE(NULLIF(bottleneck_side,''),'UNKNOWN'), COALESCE(user_type,'UNKNOWN'), COALESCE(demand_score,0) FROM filtered\
         ) summary_rows GROUP BY summary_type, segment, user_type HAVING users >= ? ORDER BY summary_type, users DESC, segment, user_type"
    );
    conn.exec_map(
        sql,
        (&run_id, access_type.clone(), access_type, keyword.clone(), keyword.clone(), keyword.clone(), keyword.clone(), keyword, req.min_value()),
        |(summary_type, segment, user_type, users, avg_demand): (String, String, String, i64, f64)| MetricCard {
            label: format!("{segment} · {user_type}"),
            value: users.to_string(),
            hint: format!("source=ads_user_experience_profile, summary_type={summary_type}, segment={segment}, user_type={user_type}, users={users}, avg_demand_score={avg_demand:.2}"),
        },
    )
    .map_err(|err| format!("failed to query full user cohort summary: {err}"))
}
