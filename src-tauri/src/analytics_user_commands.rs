...
pub fn analytics_get_user_profiles(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    ...
    if let Ok(ads_table) = batch_tables::resolve_table(
        ...
        if ads_count.unwrap_or(0) > 0 {
            ...
            return conn.exec_map(sql, (&run_id, keyword.clone(), keyword.clone(), keyword.clone(), keyword, req.min_value(), page_size, offset), |(user_key, user_type, days, gb, hours, vmos, mos, rtt, lead_type, demand, motive): (String, String, i64, f64, f64, f64, f64, f64, String, i64, i64)| {
    ...
    conn.exec_map(sql, (&run_id, &req.import_batch_id, keyword.clone(), keyword.clone(), keyword.clone(), keyword, req.min_value(), page_size, offset), |(user_key, user_type, days, gb, hours, vmos, mos, rtt, lead_type, demand, motive): (String, String, i64, f64, f64, f64, f64, f64, String, i64, i64)| {
        let label = user_key.clone();
...
pub fn analytics_get_user_summary(req: DashboardRequest) -> Result<Vec<MetricCard>, String> {
    ...
    let sql = format!(
        "SELECT summary_type, segment, user_type, CAST(COUNT(*) AS SIGNED) AS users, CAST(ROUND(AVG(demand_score),2) AS DOUBLE) AS avg_demand FROM (\
         SELECT 'demand_band' AS summary_type, CASE WHEN COALESCE(demand_score,0)>=60 THEN '60-100 高需求' WHEN COALESCE(demand_score,0)>=40 THEN '40-59 中需求' WHEN COALESCE(demand_score,0)>=20 THEN '20-39 轻需求' ELSE '0-19 低需求' END AS segment, COALESCE(user_type,'UNKNOWN') AS user_type, COALESCE(demand_score,0) AS demand_score FROM `{table}` WHERE analysis_run_id=? \
         UNION ALL \
         SELECT 'traffic_band', CASE WHEN COALESCE(total_traffic_gb,0)>=20 THEN '20GB+ 高流量' WHEN COALESCE(total_traffic_gb,0)>=10 THEN '10-20GB' WHEN COALESCE(total_traffic_gb,0)>=1 THEN '1-10GB' ELSE '<1GB' END, COALESCE(user_type,'UNKNOWN'), COALESCE(demand_score,0) FROM `{table}` WHERE analysis_run_id=? \
         UNION ALL \
         SELECT 'bottleneck_side', COALESCE(NULLIF(bottleneck_side,''),'UNKNOWN'), COALESCE(user_type,'UNKNOWN'), COALESCE(demand_score,0) FROM `{table}` WHERE analysis_run_id=?\
         ) summary_rows GROUP BY summary_type, segment, user_type ORDER BY summary_type, users DESC, segment, user_type"
    );
    conn.exec_map(
        sql,
