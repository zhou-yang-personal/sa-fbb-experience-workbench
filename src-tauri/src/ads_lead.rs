...
const S: &str = include_str!("../../database/sql/dws_to_ads/003e_analytics_lead_evidence.sql");

#[tauri::command]
pub fn ads_lead(req: EtlRequest) -> Result<CommandAck, String> {
    ...
    Ok(ack(format!("ads_lead ok: analysis_run_id={run}; affected_rows={n}")))
}
